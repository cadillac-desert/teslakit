...
def Plot_Complete(xds, show=True):
    ...
    xds - xarray.Dataset: (time) AWT, IWT, DWT, AT, Hs, Tp, Dir, SS, sea_level_pressure, wind_u, wind_v, MMSL, TWL
    '''
    ...
    # MJO
    axplot_bmus(axs[1], xds.MJO, n_cs_MJO, 'IWT')

    # DWT
    axplot_bmus(axs[2], xds.DWT, n_cs_DWT, 'DWT')

    # AT
    axplot_series(axs[3], xds.AT, 'solid', 'purple', 'AT(m)')

    # HS
    axplot_series(axs[4], xds.Hs, 'solid', 'cyan', 'Hs(m)')

    # TP
    axplot_series(axs[5], xds.Tp, 'solid', 'red', 'Tp(s)')

    # DIR
    axplot_series(axs[6], xds.Dir, 'dotted', 'black', 'Dir(º)')

    # SS
    if 'SS' in xds.variables:
        axplot_series(axs[7], xds.SS, 'solid', 'orange', 'SS(m)')
    else:
        axs[7].set_xticks([])
        axs[7].set_yticks([])
        axs[7].set_ylabel('SS(m)', rotation=0, fontweight='bold', labelpad=35)

    # SLP
    if 'SLP' in xds.variables:
        axplot_series(axs[8], xds.SLP, 'solid', 'pink', 'SLP(Pa)')
    else:
        axs[8].set_xticks([])
        axs[8].set_yticks([])
        axs[8].set_ylabel('SLP(Pa)', rotation=0, fontweight='bold', labelpad=35)

    # WNDU
    if 'WNDU' in xds.variables:
        axplot_series(axs[9], xds.WNDU, 'solid', 'brown', 'WNDU(m/s)')
    else:
        axs[9].set_xticks([])
        axs[9].set_yticks([])
        axs[9].set_ylabel('WNDU(m/s)', rotation=0, fontweight='bold', labelpad=35)

    # WNDV
    if 'WNDV' in xds.variables:
        axplot_series(axs[10], xds.WNDV, 'solid', 'olive', 'WNDV(m/s)')
    else:
        axs[10].set_xticks([])
        axs[10].set_yticks([])
        axs[10].set_ylabel('WNDV(m/s)', rotation=0, fontweight='bold', labelpad=35)

    # MMSL
    axplot_series(axs[11], xds.MMSL, 'solid', 'green', 'MMSL(m)')

    # TWL
    axplot_series(axs[12], xds.TWL, 'solid', 'blue', 'TWL(m)')
...
def Plot_Complete_sim(xds, show=True):
    '''
    Plot complete data variables inside xds
    ...
    # SLP
    if 'sea_level_pressure' in xds.variables:
        axplot_series(axs[8], xds.sea_level_pressure, 'solid', 'pink', 'SLP(Pa)')
    else:
        axs[8].set_xticks([])
        axs[8].set_yticks([])
        axs[8].set_ylabel('SLP(Pa)', rotation=0, fontweight='bold', labelpad=35)
    ...
    # WNDU
    if 'wind_u' in xds.variables:
        axplot_series(axs[9], xds.wind_u, 'solid', 'brown', 'WNDU(m/s)')
    else:
        axs[9].set_xticks([])
        axs[9].set_yticks([])
        axs[9].set_ylabel('WNDU(m/s)', rotation=0, fontweight='bold', labelpad=35)
    ...
    # WNDV
    if 'wind_v' in xds.variables:
        axplot_series(axs[10], xds.wind_v, 'solid', 'olive', 'WNDV(m/s)')
    else:
        axs[10].set_xticks([])
        axs[10].set_yticks([])
        axs[10].set_ylabel('WNDV(m/s)', rotation=0, fontweight='bold', labelpad=35)
...
###################################################################
def Plot_Complete_Nearshore(xds_near, xds_off, poi, show=True):
    '''
    Plot complete data variables inside xds

    xds_near - xarray.Dataset: (time) AWL, Hs, Tp, Dir, [& wave families but don't plot]
    xds_off - xarray.Dataset: AWT, MJO, DWT, SS, AT, MMSL, [and wave char, TWL but don't plot]
    '''
    # TODO: auto parameters inside xds
    ...
    # AWT
    axplot_bmus(axs[0], xds_off.AWT, n_cs_AWT, 'AWT')

    # MJO
    axplot_bmus(axs[1], xds_off.MJO, n_cs_MJO, 'MJO')

    # DWT
    axplot_bmus(axs[2], xds_off.DWT, n_cs_DWT, 'DWT')

    # SS
    if 'SS' in xds_off.variables:
        axplot_series(axs[4], xds_off.SS, 'solid', 'orange', 'SS(m)')
    else:
        axs[6].set_xticks([])
        axs[6].set_yticks([])
    ...
    # AT
    axplot_series(axs[3], xds_off.AT, 'solid', 'purple', 'AT(m)')

    # MMSL
    axplot_series(axs[5], xds_off.MMSL, 'solid', 'green', 'MMSL(m)')
    
    # HS
    axplot_series(axs[6], xds_near.Hs, 'solid', 'cyan', 'Hs(m)')

    # TP
    axplot_series(axs[7], xds_near.Tp, 'solid', 'red', 'Tp(s)')

    # DIR 
    xds_near.Dir
    axplot_series(axs[8], xds_near.Dir, 'dotted', 'black', 'Dir(º)')
    # suptitle
    fig.suptitle('Complete Data '.join(poi), fontweight='bold', fontsize=20)

    # reduce first 3 subplots
    gs = gridspec.GridSpec(9, 1, height_ratios = [1,1,1,3,3,3,3,3,3])
    for i in range(9):
        axs[i].set_position(gs[i].get_position(fig))
...
def Plot_PhyVariables_Histograms(data_hist, data_sim, label_1='Historical', label_2 = 'Simulation', show=True):
    'Plots histogram comparison (historical - simulation) for level related variables'

    # Compare histograms 
    Plot_FitSim_Histograms(
        data_hist, data_sim, ['Hs', 'Tp', 'Dir', 'sea_level_pressure', 'wind_u', 'wind_v'],
        color_1='white', color_2='skyblue', alpha_1=0.9, alpha_2=0.7,
        label_1= label_1, label_2 = label_2,
        gs_1 = 3, gs_2 = 2,
        density=False, show=True
