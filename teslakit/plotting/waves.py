...
def Plot_Waves_QAQC_ts(WVS):
    font = {'size'   : 14}
    plt.rc('font', **font)


    fig, ((ax1,ax2,ax3),(ax4,ax5,ax6),(ax7,ax8,ax9),(ax10,ax11,ax12)) = plt.subplots(4,3, figsize = (20, 15), sharex=True, sharey='col')
    WVS['Hs'].plot.line(ax=ax1)
    WVS['Tp'].plot.line(ax=ax2)
    WVS['Dir'].plot.line(ax=ax3)
    ax1.set_title('Hs')
    ax2.set_title('Tp')
    ax3.set_title('Dir')

    WVS['sea_Hs'].plot.line(ax=ax4)
    WVS['sea_Tp'].plot.line(ax=ax5)
    WVS['sea_Dir'].plot.line(ax=ax6)

    WVS['swell_1_Hs'].plot.line(ax=ax7)
    WVS['swell_1_Tp'].plot.line(ax=ax8)
    WVS['swell_1_Dir'].plot.line(ax=ax9)

    WVS['swell_2_Hs'].plot.line(ax=ax10)
    WVS['swell_2_Tp'].plot.line(ax=ax11)
    WVS['swell_2_Dir'].plot.line(ax=ax12)
    return fig


def Plot_Waves_QAQC_pdf(WVS):
    fig, ((ax1,ax2,ax3),(ax4,ax5,ax6),(ax7,ax8,ax9),(ax10,ax11,ax12)) = plt.subplots(4,3, figsize = (20, 15), sharex='col', sharey='col')

    sns.distplot(WVS.Hs[~np.isnan(WVS.Hs)],ax=ax1)
    sns.distplot(WVS.Tp[~np.isnan(WVS.Tp)],ax=ax2)
    sns.distplot(WVS.Dir[~np.isnan(WVS.Dir)],ax=ax3)

    ax1.set_title('Hs')
    ax2.set_title('Tp')
    ax3.set_title('Dir')

    sns.distplot(WVS.sea_Hs[~np.isnan(WVS.sea_Hs)],ax=ax4)
    sns.distplot(WVS.sea_Tp[~np.isnan(WVS.sea_Tp)],ax=ax5)
    sns.distplot(WVS.sea_Dir[~np.isnan(WVS.sea_Dir)],ax=ax6)

    sns.distplot(WVS.swell_1_Hs[~np.isnan(WVS.swell_1_Hs)],ax=ax7)
    sns.distplot(WVS.swell_1_Tp[~np.isnan(WVS.swell_1_Tp)],ax=ax8)
    sns.distplot(WVS.swell_1_Dir[~np.isnan(WVS.swell_1_Dir)],ax=ax9)

    sns.distplot(WVS.swell_2_Hs[~np.isnan(WVS.swell_2_Hs)],ax=ax10)
    sns.distplot(WVS.swell_2_Tp[~np.isnan(WVS.swell_2_Tp)],ax=ax11)
    sns.distplot(WVS.swell_2_Dir[~np.isnan(WVS.swell_2_Dir)],ax=ax12)
    return fig
