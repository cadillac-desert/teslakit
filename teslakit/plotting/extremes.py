...
def Plot_GEVParams(xda_gev_var, c_shape='bwr', c_other='hot_r', show=True):
    ...
    for c, par in enumerate(params):
        ...
        if par == 'shape':
            ...
            #norm = TwoSlopeNorm(vmin=cl[0], vcenter=0, vmax=cl[1])
        ...
        cc = ax.pcolor(rr_pv, cmap=cma, norm=plt.Normalize(vmin=cl[0], vmax=cl[1]), edgecolor='k')    
        #new script below
    ...
    for c_wt, k_wt in enumerate(sorted(d_sigma.keys())):
        for c_ch, k_ch in enumerate(sorted(d_sigma[k_wt].keys())):
            ...
            if corr.shape[0]==3:
                ...
                sigma_plot[c_wt, c_ch] = corr[0,1] # one family. Hs - Tp corr with extra var
...
def Plot_FitSim_EmpFit(data_fit, data_sim, vn, kma_fit,
                       n_bins=30,
                       color_1='white', color_2='skyblue',
                       alpha_1=0.7, alpha_2=0.4,
                       label_1='Historical', label_2 = 'Simulation',
...
def Plot_Fit_QQ(data_fit, vn, xds_GEV_Par, kma_fit, color='black',
    ...
    for c in range(n_clusters):
        ...
        # calculate MSE
        ss_res =((Q_emp-Q_gev)**2).sum() 
        mse = (ss_res/len(Q_emp))

        # customize axis
        ax.set_title('WT: {0} \nMSE = {1:.2f}'.format(wt, mse))
        ax.axis('equal')
        ax.set_xlim([-0.1, np.max([Q_emp, Q_gev], axis=None)])
        ax.set_ylim([-0.1, np.max([Q_emp, Q_gev], axis=None)])
