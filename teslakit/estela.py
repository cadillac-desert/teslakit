...
from .kma import KMA_regression_guided, KMA_simple
...
class Predictor(object):
    ...
    def Load(self):
        if op.isfile(self.p_data):
            ...
            #self.data.close()  # Close the NetCDF file after loading
        if op.isfile(self.p_pca):
            self.PCA = xr.open_dataset(self.p_pca)
            #self.PCA.close()  # Close the NetCDF file after loading
        if op.isfile(self.p_kma):
            self.KMA = xr.open_dataset(self.p_kma)
            #self.KMA.close()  # Close the NetCDF file after loading
...
#---------------------------------------------------------
    ...
    def Calc_KMA_simple(
        self, num_clusters, repres=0.95):
        'simple KMA of DWTs'

        # classification: KMA regresion guided
        repres = 0.80
        self.KMA = KMA_simple(
            self.PCA, num_clusters, repres
        )
    ...
    def Calc_KMA_regressionguided(
        ...
        #self.Save()
#***********************************************************************************************************************************
        # Directly append "_kma_regression_guided" to the file paths for windows work around
        self.p_data = f"{op.splitext(self.p_data)[0]}_kma_regression_guided{op.splitext(self.p_data)[1]}"
        self.p_pca = f"{op.splitext(self.p_pca)[0]}_kma_regression_guided{op.splitext(self.p_pca)[1]}"
        self.Save()
#***********************************************************************************************************************************
    ...
    def Mod_KMA_AddStorms(self, storm_dates, storm_categories):
        ...
        # self.KMA.to_netcdf(self.p_kma,'w')
#***********************************************************************************************
        #attempting windows work around here
        self.p_kma = f"{op.splitext(self.p_kma)[0]}_modify_kma_addstorms{op.splitext(self.p_kma)[1]}"
        self.p_data = f"{op.splitext(self.p_data)[0]}_modify_kma_addstorms{op.splitext(self.p_data)[1]}"
        self.p_pca = f"{op.splitext(self.p_pca)[0]}_modify_kma_addstorms{op.splitext(self.p_pca)[1]}"
        self.Save()
#***********************************************************************************************
