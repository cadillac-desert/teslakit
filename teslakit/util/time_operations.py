...
def npdt64todatetime(dt64):
    ...
    #print(f"ts:{ts}")
    #print(f"npdt64todatetime:{datetime(1970, 1, 1) + timedelta(seconds=ts)}")
...
#****************************************************************************************************
#added code due to lower exteme timestamps
    #lower threshold for utcfromtimestamp to work is datetime(1970, 1, 1, 1, 0, 1)
    if t0 >= datetime(1970, 1, 1, 1, 0, 1): 
        lb_dfts = lambda t: datetime.utcfromtimestamp(t)

    elif t0< datetime(1970, 1, 1, 1, 0, 1):
        #adjust t to meet the minimum threshold
        #use 10e5 to get t to equal -10e4 or greater, cannot be less
        lb_dfts = lambda t: datetime.utcfromtimestamp(t/10e5)
#****************************************************************************************************
    ...
    tg = ((tg - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's'))#/1000 #divide by 1000 incase it is in milliseconds?
    #print(f"tg:{tg}")
    tg = np_dfts(tg)
...
def date2datenum(d):
    ...
    # else get timetuple
        ...
        ttup = npdt64todatetime(d).timetuple()
        #print(f"datetime(*ttup[:6]):{datetime(*ttup[:6])}")
        #print(f"ttup:{ttup}")
