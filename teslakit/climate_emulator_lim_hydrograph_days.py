...
class Climate_Emulator(object):
    ...
    def Save(self):
        ...
        try: self.WVS_TCs.to_netcdf(self.p_WVS_TCs) # added by ML - bypass TCs
        except: pass
    ...
    def Load(self):
        ...
        try:                                                  # added 3/14/22 by ML to bypass TCs
    ...
    def Calc_StormsDates(self, xds_KMA):
        ...
        # limit storm length to 3 days added by ML
        while (np.diff(ix_ch)>3).any():
            ind_ix = np.where(np.diff(ix_ch)>3)[0]
            ix_ch = np.insert(ix_ch, ind_ix+1, ix_ch[ind_ix]+3)
        ds_ch = xds_KMA.time.values[ix_ch]  # dates where hydrograph changes
    ...
    def Calc_StormsMaxProxy(self, wvs_PROXY, lt_storm_dates):
        ...
        for d1, d2 in lt_storm_dates:
            ...
            # append data
            if np.shape(wt_PROXY_max.time.values):
                ms_PROXY.append(wt_PROXY_max.values[0])
                ms_times.append(wt_PROXY_max.time.values[0])
        ...
        for vn in vars_gev:
            ...
            for i in range(n_clusters):
                ...
                if c in cls_gbl:
                    ...
                    theta = (loc[i], sca[i])
                    try: 
                    # added by ML sept 2022 - some WT-chrm pairs contain NaNs in gumbell fit
                    # this will crash the simulation so after attempting to use gumbell fit 
                    # it will instead use the GEV method
                        ...
                        print('WT = ', c, 'gev instead of gumbel')
    ...
    def GenerateWaves(self, bmus, n_clusters, chrom, chrom_probs, sigma,
        ...
        while c < len(DWT_sim):
            ...
            if WT <= n_clusters:
                ...
                while np.isnan(corr).any()==True:
                # added by ML 8/10/2022 because Cascadia has NaNs in the corr or sigmas of these WT-ci combos
                # will pick random selections of ci until it finds a combo that does not have NaNs
                    print('WT, ci: ', WT,ci, ' has a NaN in correlation')
                    ci = choice(range(chrom.shape[0]), 1, p=pr)
                    crm = chrom[ci].astype(int).squeeze()    
                    corr = sigma[WT][int(ci)]['corr']
                    print('trying this WT, ci instead: ', WT, ci)
                    # print(corr)
                ...
                is0 = len(wvs_fams)*3 #modified by ML to correctly simulate extra vars with chromosoems (indexing error)
                for vn in vars_extra:
            ...
            # nan / negative values (only for wvs not for extra variables) # modified 3/15/22 by ML to include negative SS values)
            # if np.isnan(sim_row).any() or len(np.where(sim_row<0)[0])!=0:
            if np.isnan(sim_row).any() or len(np.where(sim_row[0:len(wvs_fams_vars)]<0)[0])!=0:
                continue
