...
def GetDistribution(xds_wps, swell_sectors):
    ...
    # fix data
    hs_fix_data = 50
    #for i in range(6): #for Roi
    for i in range(4): #for Hawaii
        phs = xds_wps['hs{0}'.format(i)].values
        p_fix = np.where(phs >= hs_fix_data)[0]

        # fix data
        xds_wps['hs{0}'.format(i)][p_fix] = np.nan
        xds_wps['tp{0}'.format(i)][p_fix] = np.nan
        xds_wps['th{0}'.format(i)][p_fix] = np.nan

    # sea (partition 0)
    sea_Hs = xds_wps['hs0'].values
    sea_Tp = xds_wps['tp0'].values
    sea_Dir = xds_wps['th0'].values
    ...
    xds_fams = xr.Dataset(
        {
            'Hs': ('time', xds_wps.hs.values[:]), # changed from hs1 (which is primary swell?) ML 1/12/2022
            'Tp': ('time', xds_wps.tp.values[:]), # changed from tp1 ML 1/12/2022
            'Dir': ('time', xds_wps.dir.values[:]), # changed from th1 ML 1/12/2022
