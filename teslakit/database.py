...
class Database(object):
    ...
    def CheckInputFiles(self):
        ...
        conf = [
            ...
            ('WAVES', ['partitions'], [op.isfile]),
            ('ESTELA', ['coastmat', 'estelamat', 'slp'],
             [op.isfile, op.isfile, op.isfile]),
            ('TIDE', ['gauge', 'hist_astro'], [op.isfile, op.isfile]),
            ('EXTRA_VARS', ['sea_level_pressure', 'storm_surge', 'wind', 'wind_u', 'wind_v'], [op.isfile, op.isfile,op.isfile, op.isfile,op.isfile]),
...
# ORIGINAL FUNCTION ABOVE, USER INPUT OPTION TESTING BELOW
    ...
    #         ('MJO', ['hist'], None),
    #         ('TCs', ['noaa'], None),
    #         ('SST', ['hist_pacific'], None),
    #         ('WAVES', ['partitions'], None),
    #         ('ESTELA', ['coastmat', 'estelamat', 'slp'], [None, None, None]),
    #         ('TIDE', ['gauge', 'hist_astro'], [None, None]),
    #         # ('HYCREWW', ['rbf_coef'], [None]),
    #         # ('NEARSHORE', ['swan_projects'], [None]),
    #     ]
    ...
    #     def select_file(k1,l_k2):
    #         filepath = filedialog.askopenfilename(title=f'Select {k1}.{l_k2} File')
    #         return filepath

    #     # Prompt user to select files for each configuration item
    #     for i, (k1, l_k2, l_files) in enumerate(conf):
    #         if l_files is None or l_files[0] is None:
    #             # Single file to select
    #             if len(l_k2) == 1:
    #                 conf[i] = (k1, l_k2, select_file(k1,l_k2[0]))
    #             # Multiple files to select
    #             elif len(l_k2) > 1:
    #                 selected_files = []
    #                 for k2 in l_k2:
    #                     selected_files.append(select_file(k1,k2))
    #                 conf[i] = (k1, l_k2, selected_files)

    #     # Update self.paths with selected file paths
    #     for k1, l_k2, l_files in conf:
    #         if l_files is not None:
    #             if isinstance(l_files, list):
    #                 for k2, filepath in zip(l_k2, l_files):
    #                     self.paths.site[k1][k2] = filepath
    #             else:
    #                 self.paths.site[k1][l_k2[0]] = l_files
    ...
    #     for k1, l_k2, _ in conf:
    #         for k2 in l_k2:
    #             fp = self.paths.site[k1][k2]
    #             rp = op.basename(fp) if fp else 'File not selected'
    #             tb.add_row([f"{k1}.{k2}", op.isfile(fp) if fp else False, rp])

    #     print(tb)
        

    def Load_SST(self):
        #print(self.paths.site.SST.hist_pacific)
    ...
    def Load_SST_PCs_sim(self):
        return xr.open_dataset(self.paths.site.SST.pcs_sim)
   ...
   # def Load_MJO_sim(self):
        #xds = xr.open_dataset(self.paths.site.MJO.sim, decode_times=True)

        ## manual fix problems with hourly time, 216 may change
    ...
    def Load_WAVES_partitions_GOW(self):
        xds = ReadGowMat(self.paths.site.WAVES.partitions)
   ...
   # def Load_TIDE_sim_astro(self):
        ...
        ## manual fix problems with hourly time, 217 may change
        #d1 = xds.time.values[0]
        #d2 = d1 + timedelta(hours=len(xds.time.values[:])-1)
        #time_fix =  generate_datetimes(d1, d2, 'datetime64[h]')
        #xds['time'] = time_fix
    ...
    def Load_storm_surge(self):
        xds = xr.open_dataset(self.paths.site.EXTRA_VARS.storm_surge)
        return xds

    def Load_wind(self):
        xds = xr.open_dataset(self.paths.site.EXTRA_VARS.wind)
        return xds

    def Load_wind_u(self):
        xds = xr.open_dataset(self.paths.site.EXTRA_VARS.wind_u)
        return xds

    def Load_wind_v(self):
        xds = xr.open_dataset(self.paths.site.EXTRA_VARS.wind_v)
        return xds

    def Load_sea_level_pressure(self):
        xds = xr.open_dataset(self.paths.site.EXTRA_VARS.sea_level_pressure)
        return xds
    ...
    def Generate_SIM_Covariates(self, total_sims=None):
        ...
        MJO_aux = self.Load_MJO_sim()
        #print(f"MJO_aux:{MJO_aux}")
        DWT = self.Load_ESTELA_DWT_sim()
        #print(f"DWT:{DWT}")
        ATD_h_aux = self.Load_TIDE_sim_astro()  # hourly data, 1 sim

        # optional select total sims
        if total_sims != None:
            AWT = AWT.isel(n_sim=slice(0, total_sims))
            MSL = MSL.isel(n_sim=slice(0, total_sims))
            MJO_aux = MJO_aux.isel(n_sim=slice(0, total_sims))
            DWT = DWT.isel(n_sim=slice(0, total_sims))
            

        # manually edit datetime for MJO and ATD to avoid time conflict with other variables, ZW
        # MJO
        time_DWT = DWT.time.values[:]
        #print(f"time_DWT:{len(DWT.time.values[:])}")
        MJO = xr.Dataset(
            {
                'evbmus_sims': (('n_sim','time'), MJO_aux.evbmus_sims.values[:].astype(int)),
            },
            coords = {'time': time_DWT}
        )
        #print(f"MJO:{len(MJO_aux.evbmus_sims.values[:].astype(int))}")

        # ATD
        DWT_h_aux = fast_reindex_hourly_nsim(DWT)
        times_DWT_h = DWT_h_aux.time.values[:]
        ATD_h = xr.Dataset(
            {
                'astro': (('time',), ATD_h_aux.astro.values[0:len(times_DWT_h)]),
            },
            coords = {'time': times_DWT_h}
        )
